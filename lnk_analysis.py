...
def file_open(path):
    ...
    file.seek(4)
    check = file.read(16)
    if != '\x01\x14\x02\x00\x00\x00\x00\x00\xc0\x00\x00\x00\x00\x00\x00F':
        print('this kind of lnk file do not analysis')
        exit()
    return file

def lnk_flag(flags_to_parse):
...
def lnk_attrib(attrib_to_parse):
    ...
    for count, items in enumerate(attrib_to_parse):
        if int(items) == 1:
            print ('attrib: '+format(attrib[count]))
            lnk_attri = print(format(attrib[count]))
...
def link_flags():
    ...
    print(file_flags)
    file_flags = lnk_flag(file_flags.bin)
...
def file_attribute():
    ...
    attributes = struct.unpack('<i', file.read(4))
    flag_atributes = BitArray(hex(attributes[0]))
    file_attribute = lnk_attrib(flag_atributes.bin)
...
def volume():
    global file
    global info_flag
    global info_header_size
                          
    linkinfo_off()
                          
    if info_flag != 'A':
        return print('VolumeIdOffset : 0 ')
...
def localbase_path():
    ...
    if info_flag != 'A':
        return print('do not have volumeid, locabasepath, locabasepathunicode')
...
##    linkinfo_off()
##    if info_flag != 'B':
##        return print('do not have netbios')
...
def machinid():
    ...
    if flag == '':
        ...
        string_off = string_size + string_off
    elif flag == '':
        file.seek(string_off)
        string_size = file.read(2)
        b = (b'\x00\x00')
    ...
    if block_signature == '\xA0\x00\x00\x03':
        ...
        print('this file do not have net bios')
